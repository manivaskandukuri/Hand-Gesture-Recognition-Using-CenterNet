...
def _compute_f1_score(true_positives, false_positives, num_annotations):
    """
    Compute the average precision, given the recall and precision curves.
    ...
    Args:
        ...
        num_annotations: Number of ground truths.
...
def _compute_confusion_matrix(all_detections, all_annotations):
    gen_size = len(all_detections)
    ...
    for i in range(gen_size):
      ...
      for j in range(num_classes):
        if (all_detections[i][j] is not None and all_detections[i][j].shape[0]>0):
          c_cm = j
        if (all_annotations[i][j] is not None and all_annotations[i][j].shape[0]>0):
          r_cm = j
      if (r_cm is not None and c_cm is not None):
        conf_matrix[r_cm][c_cm] += 1
...
def _get_detections(generator, model, score_threshold=0.1, max_detections=100, visualize=False,
                    flip_test=False,
    ...
    for i in progressbar.progressbar(range(generator.size()), prefix='Running network: '):
        ...
        if (scores[indices]>=score_threshold):
          ...
          #print("Detections after threshold:", detections)
          ...
          '''for class_id in range(generator.num_classes()):
            ...
            #print("For class-{} Class detections shape after:".format(class_id), class_detections.shape)
            all_detections[i][class_id] = class_detections
...
def evaluate(
    ...
    #print("All detection:", all_detections)
    all_annotations = _get_annotations(generator)


    # Compute Confusion matrix
    conf_matrix = _compute_confusion_matrix(all_detections, all_annotations)
    ...
    for label in range(generator.num_classes()):
        ...
        for i in range(generator.size()):
            ...
            if detections is not None:
              for d in detections:
                  ...
                  #print("Max Overlaps:",max_overlap)
        ...
        # compute F1-score
        f1_score = _compute_f1_score(true_positives, false_positives, num_annotations)
        f1_scores[label] = f1_score, num_annotations

        '''# compute Accuracy
        accuracy = _compute_accuracy(true_positives, false_positives, num_annotations)
    ...
    return average_precisions, f1_scores, conf_matrix
