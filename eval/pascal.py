...
class Evaluate(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        val_summary_writer = tf.summary.create_file_writer(self.save_path)
        with val_summary_writer.as_default():
          tf.summary.scalar('mAP', self.mean_ap, step=epoch)
          tf.summary.scalar('Mean F1-score', self.mean_f1_score, step=epoch)

        logs['mAP'] = self.mean_ap
        logs['mean_f1_score'] = self.mean_f1_score

        if self.verbose == 1:
            print('mAP: {:.4f}'.format(self.mean_ap))
            print('Mean F1-score: {:.4f}'.format(self.mean_f1_score))
